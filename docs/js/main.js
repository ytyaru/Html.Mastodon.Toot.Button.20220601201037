window.addEventListener('DOMContentLoaded', async (event) => {
    ...
    if (url.searchParams.has('code')) { // マストドンAPI oauth/authorize でリダイレクトされた場合
        ...
        console.debug('client_id:', sessionStorage.getItem('client_id'))
        console.debug('client_secret:', sessionStorage.getItem('client_secret'))
        console.debug('認証コード', code)
        // client_id, client_secretはsessionStorageに保存しておく必要がある
        const json = await tooter.getToken(sessionStorage.getItem('client_id'), sessionStorage.getItem('client_secret'), code)
        console.debug(json)
    ...
    document.getElementById('toot').addEventListener('click', async(event) => {
        ...
        if (access_token && tooter.verify(access_token)) {
            ...
            sessionStorage.setItem('client_secret', app.client_secret);
            ...
            console.debug(sessionStorage.getItem('client_id'))
            console.debug(sessionStorage.getItem('client_secret'))
